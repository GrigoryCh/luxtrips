(() => {
    ...
    function initSliders() {
        if (document.querySelector(".top__slider")) new core(".top__slider", {
            ...
            speed: 800,
            navigation: {
                prevEl: ".top__body .slider-arrow_prev",
                nextEl: ".top__body .slider-arrow_next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1.075,
        ...
        if (document.querySelector(".choose__slider")) new core(".choose__slider", {
            modules: [],
            observer: true,
            observeParents: true,
            slidesPerView: 2,
        ...
        if (window.innerWidth < 768) if (document.querySelector(".lux__slider")) new core(".lux__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 2.2,
            ...
            speed: 800,
            navigation: {
                prevEl: ".lux__container .slider-arrow_prev",
                nextEl: ".lux__container .slider-arrow_next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1.075,
    ...
    window.onload = function() {
        ...
        function displayWindowSize() {
            ...
            const slides = document.querySelectorAll(".lux__pack");
            if (w <= 768) {
                slides.forEach((elem => {
                    elem.classList.add("swiper-slide");
                }));
                initSliders();
            } else slides.forEach((elem => {
                elem.style = "";
            }));
        }
        window.addEventListener("resize", displayWindowSize);
        displayWindowSize();
        const filterCards = document.querySelectorAll(".top__slide");
        const buttonGroup = document.querySelector(".top__categories");
        if (document.querySelector(".top__categories")) buttonGroup.addEventListener("click", (event => {
            if ("SPAN" !== event.target.tagName) return false;
            buttonGroup.querySelector("._active").classList.remove("_active");
            event.target.classList.add("_active");
            const filterClass = event.target.dataset.filter;
            filterCards.forEach((elem => {
                elem.classList.remove("_hide");
                if (!elem.classList.contains(filterClass) && "all" !== filterClass) elem.classList.add("_hide");
            }));
            initSliders();
        }));
    };
    ...
    function map(n) {
        ...
        var locations = [ [ new google.maps.LatLng(46.006129, 8.950455) ] ];
        var options = {
            zoom: 12,
            panControl: false,
        ...
        var icon = {
            url: "img/icons/icon_contact.svg",
    ...
    if (document.querySelector("#map")) map(1);
    if (document.querySelector("#my-id")) {
        const picker = datepicker(document.querySelector("#my-id"), {
    ...
    document.addEventListener("click", (e => {
        ...
        if (document.querySelector(".client-tabs__items")) if (targetElelement.classList.contains("client-tabs__action")) {
            const targetDiv = targetElelement.closest(".client-tabs__item").querySelector(".client-tabs__text");
            if (!targetElelement.classList.contains("_clicked")) {
                targetElelement.classList.add("_clicked");
                targetElelement.innerHTML = "Accept";
                targetDiv.classList.add("_hidden");
                const inputElement = document.createElement("input");
                inputElement.value = targetDiv.innerHTML;
                inputElement.maxLength = "30";
                inputElement.classList.add("input-text");
                targetElelement.closest(".client-tabs__item").querySelector(".client-tabs__field").append(inputElement);
                inputElement.focus();
            } else {
                targetElelement.classList.remove("_clicked");
                targetElelement.innerHTML = "Edit";
                const inputEl = document.querySelector(".input-text");
                targetDiv.innerHTML = inputEl.value;
                inputEl.remove();
                targetDiv.classList.remove("_hidden");
            }
        } else if (targetElelement.classList.contains("client-tabs__remove")) targetElelement.closest(".client-tabs__item").remove(); else return;
    }));
    document.addEventListener("blur", (e => {
        if (document.querySelector(".input-text")) {
            const targetElelement = e.target;
            const targetDiv = targetElelement.previousElementSibling;
            const inputEl = document.querySelector(".input-text");
            if (targetElelement != inputEl) return; else {
                const targetButton = targetElelement.closest(".client-tabs__item").querySelector(".client-tabs__action");
                targetButton.innerHTML = "Edit";
                targetElelement.remove();
                targetDiv.innerHTML = inputEl.value;
                targetDiv.classList.remove("_hidden");
            }
        }
